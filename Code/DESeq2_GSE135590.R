# DESeq2 analysis for GSE135590 RNA data
library(BiocManager)
library("readr")
library(data.table)
library("DESeq2")

# RSEM read TPMs were downloaded from GEO database and used as input for this analysis
# Read the TPM data for all your samples and genes
tpms <- read.csv(file = "/data/DeSeq_GSEA_PCA/GSE135590_TPMs.csv", row.names = 2, header = TRUE)
tpms <- tpms[,6:17] #Select just the columns associated with the TPM per sample
...
write.csv(as.data.frame(resOrdered), 
          file="DEGs_GSE135590_1095genes_FDR10%.csv")
...
write.csv(as.data.frame(resOrdered2), 
          file="DEGs_GSE135590_1095genes_FDR20%.csv")
